...
-- both the universal closest request point per address, but
-- also the closest point that resides on the same street name as the @ name.
--
...
CREATE OR REPLACE FUNCTION test_find_closest_points(hood_name varchar, srid integer)
	RETURNS TABLE (addr_gid int, address varchar, req_gid int, dist double precision, req_gid_on_st int, dist_on_st double precision) AS $$
	BEGIN
